  ...
  return (
    <div className="page-stack cost-calculator-page">
      <div className="section-heading cost-calculator-page-heading">
        ...
        <p>See the full yearly cost: tuition, living costs, and fees. Not just the headline number agents quote.</p>
      </div>
      ...
      {summary ? (
        <InfoCard title="Cost summary">
          <p className="muted-text cost-summary-lead">
          ...
          <p className="muted-text cost-summary-footnote">
            Verify deposits, travel, and one-time setup costs from official sources before making decisions.
          </p>
          <Link className="text-link cost-summary-detail-link" to={`/programs/${programId}`}>
            View full program detail →
          </Link>
        </InfoCard>
      ) : (
        <div className="hub-pre-result">
          <Calculator size={56} aria-hidden="true" />
          <p>Select a program above to see the full cost breakdown.</p>
        </div>
      )}
    </div>
  )
